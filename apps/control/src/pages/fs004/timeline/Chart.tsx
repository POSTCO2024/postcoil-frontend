...
} from '@/store/fs004store';
import { transformedDataToTimelineChartData } from '@/utils/scheduling/chartUtils';
...
const Chart = ({ chartName }: PropsType) => {
  const id = 'timelineChart-' + chartName;
  ...
  useEffect(() => {
    if (materialData && materialData.length > 0) {
      const transformed = transformedDataToTimelineChartData(
        materialData,
        chartName,
      );
      setData(transformed);
  ...
  const chartOptions: Highcharts.Options = {
    chart: {
      type: 'variwide',
      width: 90 + (expectedDuration as number) * 3, // 72: y축 라벨 너비
